...
def test_simulate_multithreaded(
    LG: LangevinGillespie, nSims: int, nThreads: int
) -> tuple[bool, str]:
    try:
        a, b, c = LG.simulate_multithreaded(nSims, nThreads)
...
def run_boundary_tests(LG, param_name, test_values, nSims, nThreads):
    param_results = []
    for value, should_pass in test_values:
        setattr(LG, param_name, value)
        actual_pass, msg = test_simulate_multithreaded(LG, nSims, nThreads)
...
# Test Start
nSims, nThreads = 1000, os.cpu_count()

LG = create_valid_LG()
passed, msg = test_simulate_multithreaded(LG, nSims, nThreads)
...
LG = create_valid_LG()
passed, msg = test_simulate_multithreaded(LG, nSims, nThreads)
print_test_result("nThreads", nThreads, passed, msg, expect="failure")
...
LG, all_results["dt"] = run_boundary_tests(
    LG, "dt", [(0, False), (0.0001, True), (0.001, True), (0.01, True)], nSims, nThreads
)
...
LG, all_results["kappa"] = run_boundary_tests(
    LG, "kappa", [(-1, False), (0, True)], nSims, nThreads
)
LG, all_results["kBT"] = run_boundary_tests(
    LG, "kBT", [(-1, False), (0, True)], nSims, nThreads
)
LG, all_results["gammaB"] = run_boundary_tests(
    LG, "gammaB", [(-1, False), (1e-9, True)], nSims, nThreads
)
...
LG, all_results["transition_matrix"] = run_boundary_tests(
    LG, "transition_matrix", transition_matrices, nSims, nThreads
)

LG, all_results["initial_state"] = run_boundary_tests(
    LG, "initial_state", [(0, True), (1, True), (2, True), (3, True)], nSims, nThreads
)
