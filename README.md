...
```
F1-ATPase-simulation/
├── bin/                           # The compiled binary will be generated here from setup.py 
├── docs/                          # Project diagrams, flowcharts, roadmap
├── src/                           
...
│   │   │   └── LangevinGillespie.h     
│   │   └── binding.cpp            # C++ → Python interface
│   └── utils/                     # Helper scripts and functions
│       └── compute_transition_matrix.py # Compute transition matrix for F1-ATPase (4 States)
├── main.ipynb                     # Example simulation usage, and simulated graphs
...
> **Note**: Under the hood this project utilizes C++, CUDA, and Pybind11.
> Due to this compilation for your system is required via the `setup.py` 
> file provided as it allows you to build the binary on your own system. 
...
A compiled binary (e.g., `f1sim.cpython-313-x86_64-linux-gnu.so`) wil now be generated in the project root. Note that the exact filename will vary depending on your specific Python version and system structure.
...
The `main.ipynb` notebook provides examples on how to use the class. Additionally, a condensed example of the code will be shown below. 
<br>
...
![Single Trajectory Example](analysis/results/single_simulation_results_seed0.png)
